...
// A[B,M,K] x B[B,K,N] = C[B,M,N]
TEST(MatmulIntegrationTest, BatchedMatmul) {
  // Initialize HIP.
  ...
  // Dimensions: A[2,4,8] x B[2,8,5] = C[2,4,5]
  const int64_t B = 2;
  const int64_t M = 4;
  const int64_t K = 8;
  const int64_t N = 5;
  ...
  PinnedTensor<float> bTensor({B, K, N});
  PinnedTensor<float> cTensor({B, M, N});
  ...
  graph->set_name("batched_matmul_test");
