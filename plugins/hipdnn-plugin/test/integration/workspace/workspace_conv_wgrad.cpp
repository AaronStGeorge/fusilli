...
// Test: Workspace flow with convolution weight gradient.
// Conv wgrad produces dispatch-count > 1 after compilation, requiring non-zero
// workspace (external transient memory). This exercises:
//   - GetWorkspaceSizeFromExecutionContext returning a non-zero size
//   - ExecuteOpGraph importing a workspace buffer into the IREE runtime
TEST(IntegrationWorkspaceConvWgrad, WorkspaceNonZeroAndExecute) {
  // Initialize HIP.
  ...
  // Dimensions — matching the lit test for conv wgrad which produces
  // dispatch-count 2 on AMDGPU.
  ...
  // Initialize tensors. NHWC strides match the fusilli lit test layout.
  ...
  // Query workspace size — exercises GetWorkspaceSizeFromExecutionContext.
  // Conv wgrad with these dimensions produces dispatch-count > 1, requiring
  // external transient storage (workspace).
  ...
  // Execute graph with workspace — exercises ExecuteOpGraph workspace import.
