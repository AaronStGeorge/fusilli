...
// Test: Workspace flow with matrix multiplication.
// Exercises GetWorkspaceSizeFromExecutionContext and workspace buffer passing
// through ExecuteOpGraph. For simple matmuls the workspace size may be 0, but
// the full API path is still exercised.
TEST(IntegrationWorkspaceMatmul, WorkspaceQueryAndExecute) {
  // Initialize HIP.
  ...
  graph->set_name("workspace_matmul_test");
  ...
  // NOTE: Simple matmul graphs typically produce workspace size 0, so the
  // buffer import path in ExecuteOpGraph is not exercised by this test. A
  // graph with fused ops or larger intermediates would be needed to trigger
  // a non-zero workspace allocation.
  int64_t workspaceSize = 0;
  result = graph->get_workspace_size(workspaceSize);
  ASSERT_EQ(result.code, error_code_t::OK) << result.err_msg;
  ASSERT_GE(workspaceSize, 0) << "Workspace size should be non-negative";
