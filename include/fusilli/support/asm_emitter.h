      ...
      [&](auto val) -> std::string {
        ...
        if constexpr (std::is_same_v<T, float>) {
          return std::format(
              ...
              ": {}\n",
              resultName, std::bit_cast<uint32_t>(val), mlirType, resultType);
        } else if constexpr (std::is_same_v<T, double>) {
          return std::format(
              "\n{} = torch.vtensor.literal(dense<0x{:016X}> : tensor<1x{}>) "
              ": {}\n",
              resultName, std::bit_cast<uint64_t>(val), mlirType, resultType);
...
module @module {{
  ...
  // at the top of the function body. This way individual nodes don't need to
  // know whether their inputs are inlined scalars or regular tensors.
